...
def _setup_published(tmp_path: Path, *, title: str, content: str, metadata: dict,
                      acl_tags: list[str]) -> str:
    raw = tmp_path / "raw"
    raw.mkdir(exist_ok=True)
    store = SQLiteStore(db_path=str(tmp_path / "k.sqlite3"), data_dir=str(tmp_path))
    store.init_schema()
    doc_id = store.create_document(title, str(raw / f"{title}.md"), "text/plain",
                                    metadata, acl_tags, "v1")
...
def test_citation_outside_scope_filtered_out(tmp_path: Path) -> None:
    ...
    d_wireless = _setup_published(
        tmp_path, title="无线", content="RRC 建立失败先查告警",
...
def test_query_with_scope_or_hits_meta_value(tmp_path: Path) -> None:
    ...
    d_5g = _setup_published(
        tmp_path, title="5G", content="5G 接入处理",
        metadata={"network_type": "5g"}, acl_tags=["noc"],
    )
    store = SQLiteStore(db_path=str(tmp_path / "k.sqlite3"), data_dir=str(tmp_path))
    app = create_app(store=store)
    client = TestClient(app)
...
def test_query_no_scope_no_or_returns_all_published(tmp_path: Path) -> None:
    ...
    d = _setup_published(
        tmp_path, title="any", content="x",
        metadata={"k": "v"}, acl_tags=[],
    )
    store = SQLiteStore(db_path=str(tmp_path / "k.sqlite3"), data_dir=str(tmp_path))
    app = create_app(store=store)
    client = TestClient(app)
    r = client.post(
        "/api/v1/chat/query",
        json={"session_id": "s3", "question": "x",
              "knowledge_scopes": [], "knowledge_scopes_or": [], "stream": False},
    )
    assert r.status_code == 200
