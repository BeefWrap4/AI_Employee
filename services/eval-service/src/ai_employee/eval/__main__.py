...
from ai_employee.eval.runner import HttpApi, run
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--out", default="var/data/eval_reports",
                        help="output dir for report files")
    parser.add_argument("--timeout", type=float, default=60.0, help="per-query timeout (s)")
    parser.add_argument("--threshold-top1", type=float, default=0.6, help="top-1 hit rate threshold")
    parser.add_argument("--threshold-top3", type=float, default=0.8, help="top-3 hit rate threshold")
    parser.add_argument("--threshold-refusal", type=float, default=0.9, help="refusal accuracy threshold")
    ...
    report = build_report(
        metrics, golden_path=args.golden, api_base=args.api, top_ks=top_ks, ts=ts,
        thresholds={
    ...
    print(f"{head} RAG 评测: total={summary['total']} errored={summary['errored']} "
          f"refusal_violations={summary['refusal_violations']}")
    for k in top_ks:
        print(f"  Top-{k} 命中: {m['hit_rates'][str(k)] * 100:.0f}% ({m['hit_counts'][str(k)]}/{m['eligible_for_hit']})")
    ...
    if summary.get("refusal_expected", 0) if False else m["refusal_expected"]:
        print(f"  拒答准确: {m['refusal_accuracy'] * 100:.0f}% ({m['refusal_correct']}/{m['refusal_expected']})")
